...
var factorial = function(n) {
  ...
  // else, return the recurse of previous n integers
...
var sum = function(array) {
  ...
  // else, add the sums of each array element
...
var arraySum = function(array) {
  ...
  // if the element is an array, run the arraySum function on it and extract data
...
var sumBelow = function(n) {
  if (n === 0) {
    // when reach zero, return 0
    return 0;
  } else if (n < 0) {
    // in case where integers are negative
    return n + 1 + sumBelow(n + 1);
  } else {
    // in case where integers are positive
    return n - 1 + sumBelow(n - 1);
  }
};
...
var gcd = function(x, y) {
  // take GCD... make sure x is bigger than y
  if (y > x) {
    return gcd(y, x);
  } else if (x < 0 || y < 0) {
    return null;
